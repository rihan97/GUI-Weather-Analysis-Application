...
public class ChartSun extends Canvas{
	...
	public ChartSun(ArrayList yr, ArrayList mx, ArrayList mn, ArrayList av)
	{
		this.gYears = yr;
		this.gMax = mx;
		this.gMin = mn;
	...
	public void paint(Graphics g)
	{
		...
		mxSun = Float.parseFloat(gMax.get(0).toString());		//setting initial value of maxRainInMM
		mnSun = Float.parseFloat(gMin.get(0).toString());		//setting initial value of minRainInMM
		...
		for (i=1; i<=gYears.size()-1; i++)
		{
			...
			if ( Float.parseFloat(gMax.get(i).toString()) > mxSun )
				mxSun = Float.parseFloat(gMax.get(i).toString());			
			if ( Float.parseFloat(gMin.get(i).toString()) < mnSun )
				mnSun = Float.parseFloat(gMin.get(i).toString());
		...
		g.drawString("Sun(Hours)", 10, 300);
		//Drawing Rain(mm) Scale 
		float stepY, startY=500;
		stepY = 400 / (mxSun - mnSun);
		for (i=(int)mnSun, j=1; i<=mxSun; i++, j++)
		{
			if ( j%15==0 )
			{
