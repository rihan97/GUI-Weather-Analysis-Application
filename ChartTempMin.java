...
public class ChartTempMin extends Canvas{
	 ...
	 */
	public ChartTempMin(ArrayList yr, ArrayList mx, ArrayList mn, ArrayList av)
	{
		this.gYears = yr;
		this.gMax = mx;
		this.gMin = mn;
	...
	public void paint (Graphics g){
		...
		int mxYear, mnYear, i;
		float mxTmin, mnTmin, mxMax, mnMax;
		...
		mxTmin = Float.parseFloat(gMax.get(0).toString());		//setting initial value of maxRainInMM
		mnTmin = Float.parseFloat(gMin.get(0).toString());		//setting initial value of minRainInMM
		...
		for (i=1; i<=gYears.size()-1; i++)
		{
			...
			if ( Float.parseFloat(gMax.get(i).toString()) > mxTmin )
				mxTmin = Float.parseFloat(gMax.get(i).toString());			
			if ( Float.parseFloat(gMin.get(i).toString()) < mnTmin )
				mnTmin = Float.parseFloat(gMin.get(i).toString());
		...
		g.drawString("Temp Min(DegC)", 10, 300);
		//Drawing Temp Min(DegC) Scale 
		float stepY, startY=500;
		stepY = 400 / (mxTmin - mnTmin);
		for (i=(int)mnTmin, j=1; i<=mxTmin; i++, j++)
		{
			if ( j%15==0 )
			{
