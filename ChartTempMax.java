...
public class ChartTempMax extends Canvas{
	...
	public ChartTempMax (ArrayList yr, ArrayList mx, ArrayList mn, ArrayList av)
	{
		this.gYears = yr;
		this.gMax = mx;
		this.gMin = mn;
	...
	public void paint(Graphics g)
	{
		...
		//Scaling Lines
		//Getting minimum and maximum year and Temp Max(Deg C) values
		int mxYear, mnYear, i;
		float mxTmax, mnTmax, mxMax, mnMax;
		...
		mxTmax = Float.parseFloat(gMax.get(0).toString());		//setting initial value of maxRainInMM
		mnTmax = Float.parseFloat(gMin.get(0).toString());		//setting initial value of minRainInMM
		...
		for (i=1; i<=gYears.size()-1; i++)
		{
			...
			if ( Float.parseFloat(gMax.get(i).toString()) > mxTmax )
				mxTmax = Float.parseFloat(gMax.get(i).toString());			
			if ( Float.parseFloat(gMin.get(i).toString()) < mnTmax )
				mnTmax = Float.parseFloat(gMin.get(i).toString());
		...
		//Now Drawing Temp Max(Deg C) Scale Line
		g.drawLine(100,  500,  100,  100);
		g.drawString("Temp Max(Deg C)", 10, 300);
		//Drawing Temp Max(Deg C) Scale 
		float stepY, startY=500;
		stepY = 400 / (mxTmax - mnTmax);
		for (i=(int)mnTmax, j=1; i<=mxTmax; i++, j++)
		{
			if ( j%15==0 )
			{
