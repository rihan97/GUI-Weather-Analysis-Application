...
public class ChartAirForest extends Canvas{
	 ...
	 */
	public ChartAirForest(ArrayList yr, ArrayList mx, ArrayList mn, ArrayList av)
	{
		this.gYears = yr;
		this.gMax = mx;
		this.gMin = mn;
	...
	public void paint (Graphics g){
		...
		mxAF = Float.parseFloat(gMax.get(0).toString());		//setting initial value of maxRainInMM
		mnAF = Float.parseFloat(gMin.get(0).toString());		//setting initial value of minRainInMM
		...
		for (i=1; i<=gYears.size()-1; i++)
		{
			...
			if ( Float.parseFloat(gMax.get(i).toString()) > mxAF )
				mxAF = Float.parseFloat(gMax.get(i).toString());			
			if ( Float.parseFloat(gMin.get(i).toString()) < mnAF)
				mnAF = Float.parseFloat(gMin.get(i).toString());
		...
		g.drawString("Air Forest(Days)", 10, 300);
		//Drawing Air Forest(Days) Scale 
		float stepY, startY=500;
		stepY = 400 / (mxAF - mnAF);
		for (i=(int)mnAF, j=1; i<=mxAF; i++, j++)
		{
			if ( j%15==0 )
			{
